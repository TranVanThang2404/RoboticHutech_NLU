...
class FakeMotorUART:
    ...
    - left  : tốc độ bánh trái  (0–100)  → scale → speed_a (0–255), dir=0 (CW)
    - right : tốc độ bánh phải  (0–100)  → scale → speed_b (0–255), dir=0 (CW)
    """
    ...
    def send(self, left: int, right: int):
        ...
        if (left, right) != self._last_command:
            speed_a = round(left  / 100 * 255)
            speed_b = round(right / 100 * 255)
            frame   = _build_frame(speed_a, 0, speed_b, 0)
